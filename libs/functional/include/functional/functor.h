...
#ifdef __clang__
#define MSO_EMIT_CDECL(func, cvOpt, refOpt, noexceptOpt) \
	func(, cvOpt, refOpt, noexceptOpt)
#else
#define MSO_EMIT_CDECL(func, cvOpt, refOpt, noexceptOpt) \
	func(__cdecl, cvOpt, refOpt, noexceptOpt)
#endif

#if !defined(__clang__) && defined(_M_CEE)
#define MSO_EMIT_CLRCALL(func, cvOpt, refOpt, noexceptOpt) \
	func(__clrcall, cvOpt, refOpt, noexceptOpt)
#else
#define MSO_EMIT_CLRCALL(func, cvOpt, refOpt, noexceptOpt)
#endif
...
#define MSO_EMIT_FASTCALL(func, cvOpt, refOpt, noexceptOpt) \
	func(__fastcall, cvOpt, refOpt, noexceptOpt)
#else
#define MSO_EMIT_FASTCALL(func, cvOpt, refOpt, noexceptOpt)
#endif
...
#define MSO_EMIT_STDCALL(func, cvOpt, refOpt, noexceptOpt) \
	func(__stdcall, cvOpt, refOpt, noexceptOpt)
#else
#define MSO_EMIT_STDCALL(func, cvOpt, refOpt, noexceptOpt)
#endif
...
#define MSO_EMIT_VECTORCALL(func, cvOpt, refOpt, noexceptOpt) \
	func(__vectorcall, cvOpt, refOpt, noexceptOpt)
#else
#define MSO_EMIT_VECTORCALL(func, cvOpt, refOpt, noexceptOpt)
#endif
...
template <typename TSignature> class Functor;
template <typename TSignature> class FunctorThrow;
...
struct Decay
{
	...
	using Type = std::conditional_t<
		IsFunction<NoRefType>::value,
		std::add_pointer_t<NoRefType>,
		std::remove_cv_t<NoRefType>>;
};
...
struct ConstexprFunctorBase : public TIFunctor
{
	...
	STDMETHOD(QueryInterface)(const GUID& /*riid*/, _Outptr_ void** /*ppvObject*/) noexcept override { return E_FAIL; }
...
public:
	...
	template <typename T>
	FunctionObjectWrapper(T&& func) noexcept
...
public:
	...
	template <typename T>
	FunctionObjectWrapperThrow(T&& func) noexcept
		: m_func(std::forward<T>(func))
	{
	}
...
//! per-Functor state.
template <typename TFunc, typename TResult, typename... TArgs>
class FunctionPointerWrapper final
	: public ConstexprFunctorBase<Mso::IFunctor<TResult, TArgs...>>
{
...
template <typename TFunc, typename TResult, typename... TArgs>
class FunctionPointerWrapperThrow final
	: public ConstexprFunctorBase<Mso::IFunctorThrow<TResult, TArgs...>>
{
	static_assert(std::is_pointer<TFunc>::value, "Must be a pointer");
	static_assert(Mso::Details::IsFunction<std::remove_pointer_t<TFunc>>::value, "Must be a function pointer");
...
template <typename T> struct IsMsoFunctor : std::false_type {};
template <typename T> struct IsMsoFunctor<Mso::Functor<T>> : std::true_type {};


//! Checks if T is an Mso::FunctorThrow
template <typename T> struct IsMsoFunctorThrow : std::false_type {};
template <typename T> struct IsMsoFunctorThrow<Mso::FunctorThrow<T>> : std::true_type {};
...
private:
	template <typename T> using EnableIfIFunctor = std::enable_if_t<std::is_convertible<T*, IFunctor*>::value, int>;
	template <typename T> using EnableIfFunctionObject = std::enable_if_t<
		Mso::Details::IsFunctionObject<T, TResult, TArgs...>::Value
		&& !std::is_convertible<Mso::Details::Decay_t<T>*, IFunctor*>::value
		&& !Mso::Details::IsMsoFunctor<Mso::Details::Decay_t<T>>::value, int>;
	template <typename T> using EnableIfNoThrow = std::enable_if_t<Mso::Details::IsNoExceptFunctionObject<T, TArgs...>::Value, int>;
	template <typename T> using EnableIfThrow = std::enable_if_t<!Mso::Details::IsNoExceptFunctionObject<T, TArgs...>::Value, int>;
	template <typename T> using EnableIfStateless = std::enable_if_t<std::is_empty<T>::value, int>;
	template <typename T> using EnableIfNotStateless = std::enable_if_t<!std::is_empty<T>::value, int>;
...
public:
	...
	Functor(const Functor& other) noexcept
		: m_impl(other.m_impl)
	{
	}
	...
	_Allow_implicit_ctor_ Functor(DoNothingFunctor) noexcept
		: Functor(DoNothing())
	{
	}

	template <typename T, EnableIfIFunctor<T> = 0>
	_Allow_implicit_ctor_ Functor(_In_ T* impl, bool shouldAddRef = true) noexcept
		: m_impl(impl, shouldAddRef)
	{
	}

	template <typename T, EnableIfIFunctor<T> = 0>
	_Allow_implicit_ctor_ Functor(Mso::TCntPtr<T>& impl) noexcept
		: m_impl(impl)
	{
	}

	template <typename T, EnableIfIFunctor<T> = 0>
	_Allow_implicit_ctor_ Functor(const Mso::TCntPtr<T>& impl) noexcept
		: m_impl(impl)
	{
	}
	...
	_Allow_implicit_ctor_ Functor(T&& func) noexcept
	{
		...
		m_impl = Mso::TCntPtr<IFunctor>(const_cast<IFunctor*>(static_cast<const IFunctor*>(&s_impl)), /*shouldAddRef:*/ false);
	}

	template <typename T, EnableIfFunctionObject<T> = 0, EnableIfThrow<T> = 0>
	_SA_deprecated_(lambda must be noexcept or use Functor constructor with TerminateOnException argument or use Mso::FunctorThrow)
...
private:
	template <typename T> using EnableIfIFunctorThrow = std::enable_if_t<std::is_convertible<T*, IFunctorThrow*>::value, int>;
	template <typename T> using EnableIfFunctionObject = std::enable_if_t<
		Mso::Details::IsFunctionObject<T, TResult, TArgs...>::Value
		&& !std::is_convertible<Mso::Details::Decay_t<T>*, IFunctorThrow*>::value
		&& !Mso::Details::IsMsoFunctorThrow<Mso::Details::Decay_t<T>>::value, int>;
	template <typename T> using EnableIfStateless = std::enable_if_t<std::is_empty<T>::value, int>;
	template <typename T> using EnableIfNotStateless = std::enable_if_t<!std::is_empty<T>::value, int>;
...
public:
	...
	FunctorThrow(const FunctorThrow& other) noexcept
		: m_impl(other.m_impl)
	{
	}

	FunctorThrow(FunctorThrow&& other) noexcept
		: m_impl(std::move(other.m_impl))
	{
	}

	_Allow_implicit_ctor_ FunctorThrow(DoNothingFunctor) noexcept
		: FunctorThrow(DoNothing())
	{
	}

	template <typename T, EnableIfIFunctorThrow<T> = 0>
	_Allow_implicit_ctor_ FunctorThrow(_In_ T* impl, bool shouldAddRef = true) noexcept
		: m_impl(impl, shouldAddRef)
	{
	}

	template <typename T, EnableIfIFunctorThrow<T> = 0>
	_Allow_implicit_ctor_ FunctorThrow(Mso::TCntPtr<T>& impl) noexcept
		: m_impl(impl)
	{
	}

	template <typename T, EnableIfIFunctorThrow<T> = 0>
	_Allow_implicit_ctor_ FunctorThrow(const Mso::TCntPtr<T>& impl) noexcept
		: m_impl(impl)
	{
	}
	...
	_Allow_implicit_ctor_ FunctorThrow(T&& func) noexcept
	{
		...
		m_impl = Mso::TCntPtr<IFunctorThrow>(const_cast<IFunctorThrow*>(static_cast<const IFunctorThrow*>(&s_impl)), /*shouldAddRef:*/ false);
	}
