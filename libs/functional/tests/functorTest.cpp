...
TEST_CLASS(FunctorTest)
{
	...
	TEST_METHOD(Functor_ctor_Lambda_CaptureCopyCount)
	{
		...
		int releaseCalls = 0;
		{
			...
			data->Value = 3;
			Mso::Functor<int(int, int)> f1 = ([data](int x, int y) noexcept
			{
	...
	{
		Mso::Functor<int()> increment([i = 0]() mutable noexcept -> int
		{
			return i++;
		});
	...
	TEST_METHOD(Functor_ctor_MutableLambda_NoCapture)
	{
		static int i = 0;
		i = 0;
		Mso::Functor<int()> increment([]() mutable noexcept -> int
		{
			return i++;
		});
	...
	{
		int callCount = 0;
		auto func = std::function<void()>([&callCount]() noexcept
		{
			++callCount;
		});
	...
	{
		int callCount = 0;
		const auto func = std::function<void()>([&callCount]() noexcept
		{
			++callCount;
		});
		VoidFunctorExecutor::Execute({ func, Mso::TerminateOnException });
	...
	{
		int callCount = 0;
		auto func = std::function<void(Mso::TCntPtr<StructParam>)>([&callCount](Mso::TCntPtr<StructParam>) noexcept
		{
			++callCount;
		});
	...
	{
		int callCount = 0;
		const auto func = std::function<void(Mso::TCntPtr<StructParam>)>([&callCount](Mso::TCntPtr<StructParam>) noexcept
		{
			++callCount;
		});
		MyFunctorExecutor::Execute({ func, Mso::TerminateOnException });
	...
	{
		int bias = 5;
		auto add = Mso::Functor<int(int, int)>([bias](int x, int y) noexcept
		{
			return x + y + bias;
		});
	...
	TEST_METHOD(FunctorThrow_ctor_Lambda_CaptureCopyCount)
	{
		...
		int releaseCalls = 0;
		{
			auto data = Mso::Make<TestData>(addRefCalls, releaseCalls);
			data->Value = 3;
			Mso::FunctorThrow<int(int, int)> f1 = ([data](int x, int y) noexcept
			{
				return x + y + data->Value;
			});
	...
	{
		Mso::FunctorThrow<int()> increment([i = 0]() mutable noexcept -> int
		{
			return i++;
		});
	...
	TEST_METHOD(FunctorThrow_ctor_MutableLambda_NoCapture)
	{
		static int i = 0;
		i = 0;
		Mso::FunctorThrow<int()> increment([]() mutable noexcept -> int
		{
			return i++;
		});
	...
	{
		int bias = 5;
		auto add = Mso::FunctorThrow<int(int, int)>([bias](int x, int y) noexcept
		{
			return x + y + bias;
		});
		TestCheckEqual(15, add(4, 6));
	...
	struct CheckFunctorOverload final
	{
		...
		static int DoCheckOverload(const FunctorAlias1&) noexcept { return 1; }
		static int DoCheckOverload(const FunctorAlias2&) noexcept { return 2; }
...
TestClassComponent(FunctorNoexceptTest, Mso.Functor.Noexcept)
TEST_CLASS(FunctorNoexceptTest)
{
	...
	TEST_METHOD(Functor_noexcept_ctor_Lambda_CaptureCopyCount)
	{
		...
		int releaseCalls = 0;
		{
			auto data = Mso::Make<TestData>(addRefCalls, releaseCalls);
			data->Value = 3;
			Mso::Functor<int(int, int) noexcept> f1 = ([data](int x, int y) noexcept
			{
				return x + y + data->Value;
			});
	...
	{
		int bias = 5;
		auto add = Mso::Functor<int(int, int) noexcept>([bias](int x, int y) noexcept
		{
			return x + y + bias;
		});
		TestCheckEqual(15, add(4, 6));
