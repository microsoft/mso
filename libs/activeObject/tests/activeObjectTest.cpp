...
{
  CallableVoidFunctor(bool shouldBeCalled, char const* name) noexcept : m_shouldBeCalled{shouldBeCalled}, m_name{name}
  {
  }
...
struct TestContext
{
          ...
          | Dtor::NotCalled) noexcept
  {
    ...
    {
      case TestExpectations::CtorInQueue: OnCtor = CheckEqualQueue("Ctor"); break;
      case TestExpectations::CtorNotInQueue: OnCtor = CheckNotEqualQueue("Ctor"); break;
      default: OnCtor = CheckNotCalled("Ctor"); break;
    }

    switch (expectations & TestExpectations::InitializeNotCalled)
    {
      case TestExpectations::InitializeInQueue: OnInitialize = CheckEqualQueue("Initialize"); break;
      case TestExpectations::InitializeNotInQueue: OnInitialize = CheckNotEqualQueue("Initialize"); break;
      default: OnInitialize = CheckNotCalled("Initialize"); break;
    }

    switch (expectations & TestExpectations::FinalizeNotCalled)
    {
      case TestExpectations::FinalizeInQueue: OnFinalize = CheckEqualQueue("Finalize"); break;
      case TestExpectations::FinalizeNotInQueue: OnFinalize = CheckNotEqualQueue("Finalize"); break;
      default: OnFinalize = CheckNotCalled("Finalize"); break;
    }

    switch (expectations & TestExpectations::DtorNotCalled)
    {
      case TestExpectations::DtorInQueue: OnDtor = CheckEqualQueue("Dtor"); break;
      case TestExpectations::DtorNotInQueue: OnDtor = CheckNotEqualQueue("Dtor"); break;
      default: OnDtor = CheckNotCalled("Dtor"); break;
    }
  }
...
TEST_CLASS_EX (ActiveObjectTest, LibletAwareMemLeakDetection)
{
  ...
  TEST_METHOD(ActiveObject_VerifyIsInQueueElseCrash_Crash)
  {
  ...
  TEST_METHOD(ActiveObject_SetOnDestructedOnce_TwiceCrash)
  {
    TEST_DISABLE_MEMORY_LEAK_DETECTION();
    TestContext context{Ctor::NotInQueue | Initialize::NotCalled | Finalize::NotCalled | Dtor::NotInQueue};
    auto obj = Mso::Make<TestObject>(context);
  ...
  TEST_METHOD(ActiveField_DefaultLoadCrash)
  {
    TEST_DISABLE_MEMORY_LEAK_DETECTION();
    TestContext context;
  ...
  TEST_METHOD(ActiveField_LoadCrash)
  {
    TEST_DISABLE_MEMORY_LEAK_DETECTION();
    TestContext context;
  ...
  TEST_METHOD(ActiveField_DefaultStoreCrash)
  {
    TEST_DISABLE_MEMORY_LEAK_DETECTION();
    TestContext context;
    auto field = std::make_unique<Mso::ActiveField<std::unique_ptr<int>>>(context.Queue());
  ...
  TEST_METHOD(ActiveField_DefaultExchangeCrash)
  {
    TEST_DISABLE_MEMORY_LEAK_DETECTION();
    TestContext context;
    auto field = std::make_unique<Mso::ActiveField<std::unique_ptr<int>>>(context.Queue());
  ...
  TEST_METHOD(ActiveReadableField_DefaultLoadCrash)
  {
    TEST_DISABLE_MEMORY_LEAK_DETECTION();
    TestContext context;
    std::mutex readMutex;
  ...
  TEST_METHOD(ActiveReadableField_LoadCrash)
  {
    TEST_DISABLE_MEMORY_LEAK_DETECTION();
    TestContext context;
    std::mutex readMutex;
  ...
  TEST_METHOD(ActiveReadableField_LoadWithLockLockedCrash)
  {
    TEST_DISABLE_MEMORY_LEAK_DETECTION();
    TestContext context;
    std::mutex readMutex;
  ...
  TEST_METHOD(ActiveReadableField_LoadWithLockLockedCrash2)
  {
    TEST_DISABLE_MEMORY_LEAK_DETECTION();
    TestContext context;
    std::mutex readMutex;
  ...
  TEST_METHOD(ActiveReadableField_DefaultExchangeCrash)
  {
    TEST_DISABLE_MEMORY_LEAK_DETECTION();
    TestContext context;
    std::mutex readMutex;
  ...
  TEST_METHOD(ActiveReadableField_DefaultExchangeLockedCrash)
  {
    TEST_DISABLE_MEMORY_LEAK_DETECTION();
    TestContext context;
    std::mutex readMutex;
  ...
  TEST_METHOD(ActiveReadableField_ExchangeWithLockLockedCrash)
  {
    TEST_DISABLE_MEMORY_LEAK_DETECTION();
    TestContext context;
    context.Queue().Post([&]() noexcept {
  ...
  TEST_METHOD(ActiveReadableField_ExchangeWithLockLockedCrash2)
  {
    TEST_DISABLE_MEMORY_LEAK_DETECTION();
    TestContext context;
    context.Queue().Post([&]() noexcept {
