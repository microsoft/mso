...
*/
template < typename T, typename THelper, typename TEmptyTraits = Mso::EmptyTraits<T> > class THolder
{
...
public:
	...
	using TRefType = typename std::conditional<std::is_same<std::remove_pointer_t<TArrowType>, void>::value, Mso::NilType, std::remove_pointer_t<TArrowType>>::type;
...
*/
	...
	// STL equivalent
	void clear() noexcept { this->Clear(); } // equivalent to std::string::clear()
	void reset() noexcept { this->Clear(); } // equivalent to std::unique_ptr::reset(), excpet for not supporting (yet) passing a new pointer in (like Attach here).
...
*/
	...
	// STL equivalent
	void swap(THolder& from) noexcept { Swap(from); }

	template< typename T1, typename THelper1, typename TEmptyTraits1 > void TransferFrom(_Inout_ THolder< T1, THelper1, TEmptyTraits1 >& from) noexcept
		{
		static_assert(!Mso::RawTraits<T>::isReference, "Not allowed when T is a reference type");
		Attach(from.Detach());
		}
...
*/
	...
	// STL equivalent
	T release() noexcept { return this->Detach(); } // equivalent to std::unique_ptr::release()
...
*/
	TAddrType GetAddressOf() noexcept 
		{
		static_assert(!Mso::RawTraits<T>::isReference, "Not allowed when T is a reference type");
		AssertSzTag(IsEmpty(), "Getting the address of an existing object? This usually leads to a leak.", 0x008c269a /* tag_a9c00 */);
		return GetRaw();
		}
...
*/
	const THolder* GetThis() const noexcept { return this; }
	THolder* GetThis() noexcept { return this; }

/**
	Deprecated APIs to remove over time.
*/
	/*_SA_deprecated_(Clear)*/ void Empty() noexcept { Clear(); }
	/*_SA_deprecated_(Clear)*/ void Free() noexcept { Clear(); }
	/*_SA_deprecated_(Clear)*/ void Close() noexcept { Clear(); }
	/*_SA_deprecated_(Clear)*/ void Release() noexcept { Clear(); }
	/*_SA_deprecated_(IsEmpty)*/ bool FIsEmpty() const noexcept { return IsEmpty(); }
	/*_SA_deprecated_(Detach)*/ T Extract() noexcept { return Detach(); }
	template< typename T1 > 
	/*_SA_deprecated_(TransferFrom)*/ void Transfer(THolder< T1, THelper, TEmptyTraits >& from) noexcept { return TransferFrom( from ); }
	/*_SA_deprecated_(GetAddressOf)*/ TAddrType Ptr() noexcept { return this->GetAddressOf(); }
	/*_SA_deprecated_(ClearAndGetAddressOf)*/ TAddrType Address() noexcept { return this->ClearAndGetAddressOf(); }
...
#pragma clang diagnostic pop
	...
	// TODO: consider allowing this via extension of THelper?
	template< typename T1 > const THolder& operator=( const THolder< T1, THelper, TEmptyTraits >& from );

	// Can't mix helpers
	template< typename T1, typename THelper1, typename TEmptyTraits1 > const THolder& operator=( const THolder< T1, THelper1, TEmptyTraits1 >& from );
	template< typename T1, typename THelper1, typename TEmptyTraits1 > THolder& operator=( _Inout_ THolder< T1, THelper1, TEmptyTraits1 >&& rFrom );
	template< typename T1, typename THelper1, typename TEmptyTraits1 > void Place ( const THolder< T1, THelper1, TEmptyTraits1 >& ); 
	template< typename T1, typename THelper1, typename TEmptyTraits1 > void Attach( const THolder< T1, THelper1, TEmptyTraits1 >& ); 
	template< typename T1, typename THelper1, typename TEmptyTraits1 > void Transfer( const THolder< T1, THelper1, TEmptyTraits1 >& );

	// Improper usage
	template< typename T1 > void Place ( const THolder< T1, THelper, TEmptyTraits >& ); // use Transfer
	template< typename T1 > void Attach( const THolder< T1, THelper, TEmptyTraits >& ); // use Transfer
}; // class THolder
...
*/
template<typename T1, typename THelper1, typename TEmptyTraits1, typename T2, typename THelper2, typename TEmptyTraits2>
...
#define IMPLEMENT_THOLDER_OPERATOR_EQUALS(T) \
	template< typename T1 > const T& operator=(_In_opt_ T1 pT) noexcept { Super::operator=(pT); return *this; }
...
#define IMPLEMENT_THOLDER_RVALUE_REFS_(T, TBase) \
	...
	T& operator=( _Inout_ T&& rFrom ) noexcept { this->TransferFrom(rFrom); return *this; }
...
#define PREVENT_MISMATCH_THOLDER_CONSTRUCTORS(T) \
	template< typename T1, typename THelper > T(const THolder< T1, THelper > &ref) noexcept;
...
*/
template<typename T, typename TData> struct THolderPairData
{
public:
	operator T() const noexcept
		{ return pT; }
	T operator ->() const noexcept
		{ VerifyElseCrashTag(pT != nullptr, 0x008c288a /* tag_a9c8k */); return pT; }
...
struct EmptyTraits<THolderPairData<S, SData>>
	{
	static THolderPairData<S, SData> EmptyVal() noexcept { THolderPairData<S, SData> sEmpty = {0}; return sEmpty; }
	static bool IsEmpty(const THolderPairData<S, SData>& t) noexcept { return Mso::EmptyTraits<S>::IsEmpty(t.pT); }
	static void Empty(THolderPairData<S, SData>& t) noexcept { Mso::EmptyTraits<S>::Empty(t.pT); Mso::EmptyTraits<SData>::Empty(t.pData); }
	__declspec(deprecated) static void UnsafeEmpty(THolderPairData<S, SData>& t) noexcept { Empty(t); }
	};
...
*/
template<typename T, typename TData, typename THelper> class THolderPair : protected THolder<THolderPairData<T, TData>, THelper>
{
...
public:
	...
	using TRefType = typename std::conditional<std::is_same<std::remove_pointer_t<T>, void>::value, Mso::NilType, std::remove_pointer_t<T>>::type;

	THolderPair() noexcept {}
	explicit THolderPair(T pT, _In_opt_ TData pData) noexcept { Attach(pT, pData); }
	THolderPair(_Inout_ THolderPair&& rFrom) noexcept : Super(std::forward<Super>(rFrom)) {}
	THolderPair& operator=(_Inout_ T&& rFrom) noexcept { TransferFrom(rFrom); return *this; }

/**
	Promote basic methods from the base class
*/
	void Clear() noexcept { return Super::Clear(); }
	void clear() noexcept { return Super::Clear(); }
	bool IsEmpty() const noexcept { return Super::IsEmpty(); }

	T Get() const noexcept { return Super::Get(); }
	T get() const noexcept { return Super::get(); }

#ifdef MSO_THOLDER_EXPLICIT_GET_ONLY
	explicit operator bool() const noexcept { return Super::operator bool(); }
#else
	/*_SA_deprecated_(Get)*/ operator T() const noexcept { return Get(); }
#endif

	TRefType& operator[](ptrdiff_t iSubscript) noexcept { return Super::Get()[iSubscript]; }
	T operator ->() const noexcept { return Super::operator ->(); }

	void Swap(THolderPair& from ) noexcept { return Super::Swap(from); }
	...
	{
		AssertSzTag(IsEmpty(), "Getting the address of an existing object? This usually leads to a leak.", 0x008c269c /* tag_a9c02 */);
...
*/
	TData GetData() const noexcept { return this->m_pT.pData; }


	/*_SA_deprecated_(Clear)*/ void Empty() noexcept { return Super::Clear(); }
};
