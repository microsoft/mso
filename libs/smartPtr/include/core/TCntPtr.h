...
class TCntPtrRef
{
	...
	template <typename U> friend class Mso::TCntPtr;

public:
	operator Mso::TCntPtr<T>*() noexcept{ return m_pTCntPtr; }
	operator void*() noexcept { return m_pTCntPtr; }
	operator T**() noexcept { return m_pTCntPtr->GetAddressOf(); }
	operator void**() noexcept { return reinterpret_cast<void**>(m_pTCntPtr->GetAddressOf()); }
	operator const void**() noexcept { return reinterpret_cast<const void**>(const_cast<const T**>(m_pTCntPtr->GetAddressOf())); }
	...
	T*& operator*() noexcept { return *(m_pTCntPtr->GetAddressOf()); }
	T** GetRaw() const noexcept { return m_pTCntPtr->GetRaw(); }
	T* const* GetAddressOf() const noexcept { return m_pTCntPtr->GetAddressOf(); }
	T** ClearAndGetAddressOf() noexcept { return m_pTCntPtr->ClearAndGetAddressOf(); }
...
public:
	...
	{
		if (fDoAddRef) CheckedAddRef(m_pT);
	}
...
private:
	...
	template <typename TOther> friend class TCntRef;
};
...
public:
	...
	*/
	...
	{
		if (fDoAddRef) Details::TCntPtrAddRefStrategyImpl<TAddRefStrategy::Strategy>::CheckedAddRefOnNewlyAssignedPtr(&m_pT);
	}
	...
	// Delete this constructor. Developers must be explicit in their code when they want to use unsafe conversion from void* or from a base class.
	// Use TCntPtr<T>(static_cast<T*>(ptr)) or qi_cast<T>(ptr) instead.
	template <typename TOther, typename = typename std::enable_if<!std::is_base_of<T, TOther>::value>::type>
	...
	template <typename TOther>
	TCntPtr(const Mso::TCntRef<TOther>& from) noexcept 
		: TCntPtr{from.Ptr()}
	{
	}
	...
	*/
	...
	const TCntPtr* GetThis() const noexcept { return this; }
	TCntPtr* GetThis() noexcept { return this; }
	...
	*/
	...
	{
		AssertSzTag(IsEmpty(), "Getting the address of an existing object? This usually leads to a leak.", 0x0152139c /* tag_bu7o2 */);
		return GetRaw();
	}
	...
	*/
	/*_SA_deprecated_(Get)*/ T* get() const noexcept { return this->Get(); }
	/*_SA_deprecated_(Swap)*/ void swap(TCntPtr& from) noexcept { Swap(from); }
	/*_SA_deprecated_(Clear)*/ void clear() noexcept { this->Clear(); }
	...
	template <typename TOther>
	/*_SA_deprecated_(move assignment)*/ void TransferFrom(_Inout_ TCntPtr<TOther>& from) noexcept { Attach(from.Detach()); }
	template <typename TOther>
	/*_SA_deprecated_(move assignment)*/ void Transfer(TCntPtr<TOther>& from) noexcept { return TransferFrom(from); }
