...
*/
template< typename T > struct TDeleteHelper
{
...
*/
template <typename T> class TOwnerPtr : public THolder<T*, TDeleteHelper<T*>>
{
...
public:
	...
	T* release() noexcept { return Super::Detach(); }
	void reset(T* pT) noexcept { this = pT; }
...
/**
	Manages a pointer allocated with new[]
*/
template< typename T > struct TArrayHelper
{
	static void Free(T pT) noexcept 
	{ 
		delete[] pT;
...
*/
template <typename T> class TArrayHolder : public THolder<T*, TArrayHelper<T*>>
{
...
*/
template< typename T > struct TFreeHelper
{
	static void Free(T pT) noexcept 
	{
		pT->Free();
...
*/
template <typename T> class TOwnerIPtr : public THolder<T*, TFreeHelper<T*>>
{
...
*/
template <typename T> struct TDestroyHelper
{
	static void Free(T pT) noexcept 
	{
		pT->Destroy();
...
/**
	...
	method and use std::unique_ptr, std::shared_ptr, etc.
*/
template <typename T> class TDestroyPtr : public THolder<T*, TDestroyHelper<T*>>
{
