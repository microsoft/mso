...
namespace TaggingUtilities
{
	...
	template <typename T>
	_Success_(return == true) bool ConvertTagToStringInternal(uint32_t iTag, _Out_z_cap_(cch) T *result, size_t cch) noexcept
	{
	...
	*/
	template<size_t cch> inline void ConvertTagToString(uint32_t iTag, _Out_ _Post_z_ char (&szOut)[cch]) noexcept
		{
		static_assert(cch >= MAX_TAG_CCH, "buffer is too small");
		(void) ConvertiTagToSzTag(iTag, szOut, cch);
		}

	template<size_t cch> inline void ConvertTagToString(uint32_t iTag, _Out_ _Post_z_ wchar_t (&wzOut)[cch]) noexcept
		{
		static_assert(cch >= MAX_TAG_CCH, "buffer is too small");
		(void) ConvertiTagToWzTag(iTag, wzOut, cch);
		}

	template<size_t cch> inline void ConvertTagToString(uint32_t iTag, _Out_ _Post_z_ char16_t (&wzOut)[cch]) noexcept
		{
		static_assert(cch >= MAX_TAG_CCH, "buffer is too small");
		(void) ConvertTagToStringInternal(iTag, wzOut, cch);
		}

	template<size_t cch> inline void ConvertTagToString(uint32_t iTag, _Out_ _Post_z_ char32_t (&wzOut)[cch]) noexcept
		{
		static_assert(cch >= MAX_TAG_CCH, "buffer is too small");
		(void) ConvertTagToStringInternal(iTag, wzOut, cch);
		}
	...
	*/
	...
	inline uint32_t ConvertStringToTagInternal(_In_count_(cch) const T* wz, size_t cch) noexcept
	{
		...
		if(5 == cch)
		{
			return static_cast<uint32_t>(((((((((details::TagLetterConvertValue(wz[0])) <<6) +
							details::TagLetterConvertValue(wz[1]))<<6) +
							details::TagLetterConvertValue(wz[2]))<<6) +
							details::TagLetterConvertValue(wz[3]))<<6) +
