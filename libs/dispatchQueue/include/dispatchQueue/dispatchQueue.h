...
template <typename TInvoke, typename TOnCancel>
struct DispatchTaskImpl final : Mso::UnknownObject<Mso::QueryCastHidden<Mso::IVoidFunctor>, Mso::ICancellationListener>
{
...
{
  if (!tlsValue)
  { // Delete the queue local value when tlsValue is null.
    if (*localValue)
    {
  ...
  { // Initialize queue local value on the first call
...
inline DispatchQueue::DispatchQueue(IDispatchQueueService* state, Mso::AttachTagType tag) noexcept : m_state{state, tag}
{
}
...
inline DispatchTaskBatch::DispatchTaskBatch(Mso::CntPtr<IDispatchQueueService> const& state) noexcept : m_state{state}
{
}
