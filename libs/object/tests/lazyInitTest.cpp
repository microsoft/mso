...
private:
	explicit CountedType() noexcept
		: m_callCount{ ++s_callCount }
	{
	}

	explicit CountedType(int argument0) noexcept
		: m_callCount{ ++s_callCount }, m_argument0{ argument0 }
	{
	}
...
private:
	...
	explicit UnknownType(int argument0) noexcept
		: m_argument0{ argument0 }
	{
	}

	explicit UnknownType(int argument0, char argument1) noexcept
		: m_argument0{ argument0 }, m_argument1{ argument1 }
	{
	}
...
TEST_CLASS(LazyInitTests)
{
	TEST_METHOD(Get_CalledMultipleTimesForCountedType_ReturnsSameObject)
	{
		...
		TestAssert::AreEqual((void*)&(instance.Get()), (void*)pointer, L"Instance should always be the same when calling Get() the second time.");
		TestAssert::AreEqual((void*)&(instance.Get()), (void*)pointer, L"Instance should always be the same when calling Get() the third time.");
	...
	TEST_METHOD(GetWith1Parameter_CalledMultipleTimesForCountedType_ReturnsSameObject)
	{
		...
		TestAssert::AreEqual((void*)&(instance.Get()), (void*)pointer, L"Instance should always be the same when calling Get() the second time.");
		TestAssert::AreEqual((void*)&(instance.Get(1, 'A')), (void*)pointer, L"Instance should always be the same when calling Get() the third time.");
	...
	TEST_METHOD(GetWith2Parameters_CalledMultipleTimesForCountedType_ReturnsSameObject)
	{
		...
		TestAssert::AreEqual((void*)&(instance.Get(1)), (void*)pointer, L"Instance should always be the same when calling Get() the second time.");
		TestAssert::AreEqual((void*)&(instance.Get()), (void*)pointer, L"Instance should always be the same when calling Get() the third time.");
	...
	TEST_METHOD(Get_InParallelForCountedType_ReturnsSameObject)
	{
		...
		{
			threads[i] = std::thread([&](const size_t index) noexcept -> void
			{
		...
		for (size_t i{ 0 }; i < results.size(); ++i)
		{
			TestAssert::AreEqual((void*)results[i], (void*)results[0]);// , L"Call '%zu' to Get() should return the same value as the first call.", i);
		}
	}
	...
	TEST_METHOD(Release_InParallelForCountedType_SilentlySucceeds)
	{
		...
		{
			threads[i] = std::thread([&]() noexcept -> void
			{
				instance.Release();
			});
		}
