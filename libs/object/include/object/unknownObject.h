...
struct QueryInterfaceHelper
{
	...
	static HRESULT QueryInterface(T* obj, const GUID& riid, _Outptr_ void** ppvObject) noexcept
	{
		...
		{
			VerifyElseCrashSzTag(*ppvObject == nullptr, "*ppvObject must be null to avoid memory leaks.", 0x01003718 /* tag_bad2y */);
		}
#endif

		// QueryCastBridge is used to QI for an interface without AddRef
		const GUID& intfGuid = (riid == __uuidof(QueryCastBridge))
			? reinterpret_cast<QueryCastBridge*>(ppvObject)->ObjectId
			: riid;
...
/**
	...
	11) A class that implements IRefCounted with a custom stateless allocator.
			...
			class Foo : public Mso::UnknownObject<Mso::SimpleRefCountPolicy<Mso::DefaultRefCountedDeleter, FooAllocator>, ISomeComInterface>
			{
				...
			};
...
class DECLSPEC_NOVTABLE UnknownObject
...
public:
	...
	STDMETHOD_(ULONG, Release)() noexcept override
	{
		...
		Debug(VerifyElseCrashSzTag(static_cast<int32_t>(refCount) >= 0, "Ref count must not be negative.", 0x011055a0 /* tag_befw6 */));
		if (refCount == 0)
		{
...
public:
	...
	_Success_(return == S_OK)
	STDMETHOD(QueryInterface)(const GUID& riid, _Outptr_ void** ppvObject) override
	{
		return ::Mso::Details::QueryInterfaceHelper<UnknownObject>::QueryInterface(this, riid, ppvObject);
	}
	...
	{
		const uint32_t refCount = --m_refCount;
		Debug(VerifyElseCrashSzTag(static_cast<int32_t>(refCount) >= 0, "Ref count must not be negative.", 0x011055a2 /* tag_befw8 */));
		if (refCount == 0)
		{
...
public:
	...
	{
		const uint32_t refCount = --m_refCount;
		Debug(VerifyElseCrashSzTag(static_cast<int32_t>(refCount) >= 0, "Ref count must not be negative.", 0x011055c0 /* tag_befxa */));
		if (refCount == 0)
		{
			RefCountPolicy::Delete(this);
		}
...
*/
template <typename... TBaseTypes>
class AgileUnknownObject
	: public UnknownObject<TBaseTypes...>
{
	...
	STDMETHOD(QueryInterface)(const GUID& riid, _Outptr_ void** ppvObject) noexcept override
	{
		...
		if (riid == __uuidof(IMarshal) || riid == __uuidof(IAgileObject))
		{
			if (m_ftm.IsEmpty())
			{
				...
				VerifySucceededElseCrashTag(CoCreateFreeThreadedMarshaler(this->template StaticCastElseNull<IUnknown*>(), &ftm), 0x01003719 /* tag_bad2z */);
				// Only assign ftm to m_ftm if its value is nullptr. Otherwise we will delete the new ftm and use the existing m_ftm.
